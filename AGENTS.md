...
## Absolute Rules (Codex must follow these without exception)
...
- **MCP server**: `@coding-solo/godot-mcp` (local scope — activate with `Codex mcp add godot --scope local -- npx @coding-solo/godot-mcp`)
...
Slash commands live in `.Codex/commands/*.md`.
...
- **`Codex.yml`** — enables `@Codex` in PRs and Issues
